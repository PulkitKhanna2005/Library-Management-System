...
def issuer_card():
	Cid = sd.askstring('Issuer Card ID Pop-Up', 'What is the Issuer\'s Card ID?\t\t\t')

	if not Cid:
		mb.showerror('Invalid Issuer ID Pop-Up', 'Can\'t keep Issuer ID empty ; It must have a value')
	else:
		return Cid
...
def add_record():
	...
	if bk_status.get() == 'Issued':
		...
		card_id.set('N/A')
	surety = mb.askyesno('Add Book Details Pop-Up',
	...
	if surety:
		try:
			...
			connector.commit()
			clear_and_display()
			mb.showinfo('Record Addition Pop-Up', 'The new record was successfully added to your database')
		except sqlite3.IntegrityError:
			mb.showerror('Book ID Already in Use!',
			             'The Book ID you are trying to enter is already in the database. Please use a differnt ID.')
...
def remove_record():
	...
	mb.showinfo('Record Deletion Pop-Up', 'The record was successfully deleted.')
	clear_and_display()
	
def delete_inventory():
	if mb.askyesno('Clear Inventory Pop-Up', 'Are you sure you want to clear the entire Inventory data?\n\nThis command cannot be reversed'):
		tree.delete(*tree.get_children())
...
def change_availability():
	...
	if BK_status == 'Issued':
		surety = mb.askyesno('Book Return Pop-Up', 'Has the book been returned to the library?')
		if surety:
			cursor.execute('UPDATE Library SET bk_status=?, card_id=? WHERE bk_id=?', ('Available', 'N/A', BK_id))
			connector.commit()
		else: mb.showinfo(
			'Update Info Pop-Up', 'The book status cannot be set to Available unless it has been returned')
	else:
...
root = Tk()
root.title("Pulkit's Library Management System v.1.0")
root.geometry('1010x510')
root.resizable(0, 0)
Label(root, text="PULKIT's LIBRARY MANAGEMENT SYSTEM", font=("Noto Sans CJK TC", 15, 'bold'), bg=btn_hlb_bg, fg='White').pack(side=TOP, fill=X)
bk_status = StringVar()
bk_name = StringVar()
bk_id = StringVar()
...
Label(left_frame, text='Book Name', bg=lf_bg, font=lbl_font).place(x=90, y=25)
Entry(left_frame, width=25, font=entry_font, text=bk_name).place(x=45, y=55)
Label(left_frame, text='Book ID', bg=lf_bg, font=lbl_font).place(x=100, y=105)
bk_id_entry = Entry(left_frame, width=25, font=entry_font, text=bk_id)
bk_id_entry.place(x=45, y=135)
Label(left_frame, text="Author's Name", bg=lf_bg, font=lbl_font).place(x=80, y=185)
Entry(left_frame, width=25, font=entry_font, text=author_name).place(x=45, y=215)
Label(left_frame, text='Book Status', bg=lf_bg, font=lbl_font).place(x=90, y=265)
...
Button(RT_frame, text='Delete Book Record', font=btn_font, bg=btn_hlb_bg, fg='White',width=17, command=remove_record).place(x=8, y=30)
Button(RT_frame, text='Delete All Records', font=btn_font, bg=btn_hlb_bg, fg='White',width=17, command=delete_inventory).place(x=178, y=30)
Button(RT_frame, text='Update Book Details', font=btn_font, bg=btn_hlb_bg,fg='White', width=17,
       command=update_record).place(x=348, y=30)
Button(RT_frame, text='Toggle Availability', font=btn_font, bg=btn_hlb_bg,fg='White', width=19,
       command=change_availability).place(x=518, y=30)
Label(RB_frame, text='INVENTORY', bg=rbf_bg, fg='White',font=("Noto Sans CJK TC", 15, 'bold')).pack(side=TOP, fill=X)
...
tree.heading('Author', text="Author's Name", anchor=CENTER)
tree.heading('Status', text='Book Status', anchor=CENTER)
tree.heading('Issuer Card ID', text="Issuer's Card ID", anchor=CENTER)
